...
> **写入操作（create/update）** 使用 `字段编号`（如 `DEA001`）
...
| 字段编号 | 节点名称(别名) | 名称 | 说明 |
| --- | --- | --- | --- |
| `DEA001` | `item_no` | 品号 | 主键 |
| `DEA002` | `product_name` | 品名 | |
| `DEA003` | `inventory_unit` | 库存单位 | |
| `DEA004` | `barcode_no` | 条码编号 | |
| `DEA005` | `product_category` | 商品分类 | |
| `DEA006` | `accounting_classification` | 会计分类 | |
| `DEA007` | `calc_inventory` | (预留)计算库存 | |
| `DEA008` | `main_warehouse` | 主要仓库 | |
| `DEA009` | `main_source` | 主要来源 | |
| `DEA010` | `main_supplier` | 主供应商 | |
...
- **写入（create/update）**：使用 `字段编号` 列（如 `DEA001`）
- **读取响应（getMultiple）**：同时返回 `字段编号` 和 `节点名称`
- **条件查询（fastquery）**：条件中的字段名使用 `节点名称`（别名）
- **审批/删除**：通过 `datakeys` 定位，使用主键 `DEA001`
