...
/**
 * Helper to send JSON-RPC messages to the MCP server via stdio
...
function spawnServer(): {
  ...
  const child = spawn("node", [DIST_INDEX], {
    ...
    env: { ...process.env, YZCLI_PYTHON: "python" },
  });
  ...
  // Give time for responses then resolve
...
describe("MCP Server integration", () => {
  it("responds to tools/list with 5 tools", async () => {
    ...
    try {
      ...
      // Wait a bit for initialize response, then list tools
